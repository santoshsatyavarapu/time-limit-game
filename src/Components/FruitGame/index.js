...
class FruitGame extends Component {
  ...
  getActiveImageUrl = () => {
    ...
    const filteredImages = imagesList.filter(
      eachImageDetails => eachImageDetails.id === activeId,
    )
    return filteredImages[0].imageUrl
  }

  imageSelected = id => {
    const {activeId, score} = this.state
    if (activeId === id) {
      this.setState({score: score + 1})
      const randomIndex = Math.floor(Math.random() * imagesList.length)
      const item = imagesList[randomIndex]
  ...
  render() {
    const {activeTabId, score, gameStatus, timeLeft} = this.state

    const filteredList = this.getFilteredProjects()

    const activeImageUrl = this.getActiveImageUrl()
    ...
    return (
      <>
        ...
        <div className="container">
          {gameStatus && (
            <>
              <img src={activeImageUrl} alt="match" className="game-image" />
